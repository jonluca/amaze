...
**TestFlight 1.0.0 (6)** includes [larger mazes](MAZE_GROWTH.md), the [freeform board design](OPEN_BOARD_DESIGN.md), and [more reliable swipe direction](SWIPE_DIRECTION.md). Classic grows from 5×5 to 16×16, with stronger branching and backtracking requirements through level 200. Time Rush follows the same progression across five-maze rounds. See [release verification](VALIDATION.md).
...
Build **1.0.0 (6)** is the latest private TestFlight build. Build 3 remains attached to the App Store version waiting for review. The app uses native `TabView` navigation, `NavigationStack` toolbars, a segmented mode `Picker`, and `List`/`Form` screens. Accessibility text sizes keep the board visible while the controls scroll. The more top-down camera fits the complete freeform board, using approximately 84–86% of the available width when height permits. Texture generation runs away from the UI actor, the scene waits for prepared resources and its first rendered frame, and the launch screen uses the app's dark background. Ordered move events preserve rapid turns through display updates; old touches and callbacks cannot affect a reset run. The replacement [generated icon and its prompt](Design/README.md) are included.

- **Classic:** unlimited swipes and deterministic levels growing from 5×5 to 16×16. Larger mazes require more independent path coverage, decisions, and backtracking. Existing saved boards retain their geometry.
...
- **Worlds:** Aurora, Timber, Porcelain, and Midnight themes with freeform contour walls, transparent cutouts, visible path grids, continuous paint, reflective textured balls, animated coins, and native 3D collection previews.
...
For a physical device, choose your development team in Signing & Capabilities. The project contains the Prism Roll bundle and team identifiers; a separate publisher needs its own signing, App Store, and advertising configuration. Prism Roll's App Store record and No Ads product have been created. Version **1.0.0 (6)** has a verified production archive built on GitHub with Xcode **26.6 (17F113)** and the **iOS 26.5 SDK**, followed by local distribution signing and export. Apple processed it as **VALID** and **APP_STORE_ELIGIBLE**. Upload, processing, and review status are tracked in [VALIDATION.md](VALIDATION.md).

Build 6 is **In Beta Testing** in the private **Owner Testing** group with the sole existing owner tester and automatic notifications enabled. Builds 2, 3, 4, and 5 remain available in that group. The App Store version remains attached to build 3 and **Waiting for Review**, verified at 12:18 PM Pacific on September 7. The No Ads review submission is documented in [VALIDATION.md](VALIDATION.md). All twelve replacement screenshots and the native No Ads review image are complete with matching server checksums. Release remains manual after approval. Production AdMob identifiers and published privacy messages are configured; sample-ad requests and US opt-out were exercised, while production serving and European consent delivery remain unverified. The production archive contains the app and both Google SDK privacy manifests. The public [support](https://thoughtahead.com/prism-roll/support.html) and [privacy policy](https://thoughtahead.com/prism-roll/privacy.html) URLs are verified; see [VALIDATION.md](VALIDATION.md) for release evidence and service limits.
...
[Build 6's successful run](https://github.com/jonluca/amaze/actions/runs/34154570526) archived source commit `32fbf570ab38696e970dd4e3de5a2877215b174f`. The final IPA passed signature, entitlement, ZIP integrity, privacy-manifest, and app/dSYM UUID checks; no test artifacts were packaged, and the production SDK metadata remained unchanged. Its verified SHA-256 is recorded in [VALIDATION.md](VALIDATION.md).
...
Use iOS **26.1** for the shared scheme's local StoreKit tests; the same fixture encountered a StoreKit runtime failure on iOS 26.5. Use a destination UDID if several simulators match. The local fixture uses simulated pricing and does not exercise App Store Connect purchases. A clean export of the exact build 6 source commit passed all 80 engine/input tests, including its committed source symlinks. Final integration of the new maze generation, rendering, and input passed 37 native checks and five gameplay UI flows. The earlier build 5 swipe and smoothness checkpoint passed 178 native tests and five UI tests; see [SMOOTHNESS.md](SMOOTHNESS.md). These counts overlap.

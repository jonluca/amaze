...
Build **1.0.0 (10)** is the latest private TestFlight build. The App Store version remains attached to build 8 and waiting for review; its existing No Ads submission is recorded in [VALIDATION.md](VALIDATION.md#current-app-store-review--build-8). The app uses native `TabView` navigation, `NavigationStack` toolbars, a segmented mode `Picker`, and `List`/`Form` screens. Accessibility text sizes keep the board visible while the controls scroll. The more top-down camera fits the complete freeform board, using approximately 84–86% of the available width when height permits. Texture generation runs away from the UI actor, the scene waits for prepared resources and its first rendered frame, and the launch screen uses the app's dark background. Ordered move events preserve rapid turns through display updates; old touches and callbacks cannot affect a reset run. The replacement [generated icon and its prompt](Design/README.md) are included.
...
[Build 10's successful run](https://github.com/jonluca/amaze/actions/runs/34309631839) archived source commit `2deb7b13f65704d5cdf737e675194105f50c59f9`. The final IPA passed primary and independent signature, entitlement, ZIP integrity, privacy-manifest, and app/dSYM UUID checks; no test artifacts were packaged, and the production SDK metadata remained unchanged. Its verified SHA-256 is recorded in [VALIDATION.md](VALIDATION.md#build-10-testflight-release).

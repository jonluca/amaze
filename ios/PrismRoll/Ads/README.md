...
- Call `presentInterstitial(onDismiss:)` exactly once from each completed level's Continue action. Advance in `onDismiss`. Each launch starts with at least eight completed levels and three minutes after the first completion before an automatic ad is eligible. Later opportunities require four completions and, after any full-screen ad, at least 90 seconds since dismissal. The 90-second minimum is this app's product choice, not a Google-mandated interval. Unavailable ads call the continuation immediately. Ads never appear from a delayed load callback, during gameplay, at launch, or after failure/retry.
...
- Hide ad-dependent reward buttons while inventory is loading or unavailable; show “Watch ad” only when `canShowRewarded` is true. Free hints remain available on unfinished Classic levels 1–5 with a “First 5 levels” caption. Existing lifecycle calls to `prepare()` retry inventory without a visible retry button. Keep `statusMessage` available in Settings for more detail.
...
5. Validate consent-required, consent-not-required, revoked/changed consent, offline, no-fill, early dismissal, earned reward, presentation failure, background/foreground, and initial eight-level/three-minute grace followed by four-level interstitial cadence on device. Real ad serving still depends on Google's account approval, privacy configuration, network access, and inventory.

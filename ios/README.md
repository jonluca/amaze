...
**1.1.0 (15)** is **Waiting for Review**, verified **September 16, 2026 at 4:54 PM Pacific**. The submission includes the app, 12 Game Center achievements, five leaderboards, and three activities. It replaces the earlier build 14 submission and is configured to release automatically after approval (`AFTER_APPROVAL`). Build 15 combines the growth features, [Game Center integration](GameCenter/README.md), and optional [crash reporting](DIAGNOSTICS.md). These additions are pending approval, not publicly available. See [build 15 review verification](VALIDATION.md#build-15-app-store-review).

The live App Store version is **1.0.0 (12)**. Build 12 added 1,000 unique Classic boards with bundled exact Perfect counts, asynchronous optimal hints from any current paint state, and continuous drag turns and reversals. The level catalog uses the current board definitions directly. See [build 12 release history](VALIDATION.md#build-12-testflight-and-app-store-review).

**1.0.0 (13)** was verified in the private **Owner Testing** TestFlight group on **September 9, 2026 at 4:12 PM Pacific**. Its continuous dragging requires clearer, more deliberate turns and ignores release movement that could add an accidental second action. Those changes are retained in build 15. See [build 13 release history](VALIDATION.md#build-13-testflight-release).

The build also includes the [Coin Shop and advancing milestones](COINS_MILESTONES.md), 18 balls across six rarities, paid unlocks from 500 to 150,000 coins, distinct trails, smoother rolling, completion effects, and stronger haptics. Tap the wallet or **Get coins** in Collection for coin packs and optional 50-coin videos.
...
Swipe up, down, left, or right anywhere in the gameplay area, including the heading and empty space around the board. The native touch recognizer starts once a stroke travels 8 points with a clear direction, before the finger lifts. Keep the finger down and drag to turn or reverse; subsequent turns need 24 points of clear directional travel. Small hooks and diagonal drift retain the current direction, and lifting the finger cannot add another turn. Buttons, tabs, pickers, scrolling controls, and presented sheets keep their normal interactions. The ball rolls until a wall stops it, painting every tile it crosses. Paint all open tiles to advance automatically. Blocked swipes do not count as moves.
...
The app uses native `TabView` navigation, `NavigationStack` toolbars, a segmented mode `Picker`, and `List`/`Form` screens. Accessibility text sizes keep the board visible while the controls scroll. The more top-down camera fits the complete freeform board, using approximately 84–86% of the available width when height permits. Texture generation runs away from the UI actor, the scene waits for prepared resources and its first rendered frame, and the launch screen uses the app's dark background. Ordered move events preserve rapid turns through display updates; old touches and callbacks cannot affect a reset run. The replacement [generated icon and its prompt](Design/README.md) are included.
...
- **Game Center:** build 15 adds 12 achievements, five cumulative leaderboards, and Apple Games activities for Classic, Time Rush, and Daily Maze. Reporting follows completed gameplay and supports queued retries. The catalog and app are awaiting review together; authenticated GameKit reporting on a device remains unverified. See [Game Center](GameCenter/README.md).
...
Open `PrismRoll.xcodeproj`, select the **PrismRoll** scheme and an iPhone/iPad simulator, then Run. Dependencies are pinned in `Package.resolved`, including Google Mobile Ads 13.9.0, User Messaging Platform 3.1.0, Firebase 12.19.2, and SnapshotTesting 1.19.5 for tests. Deployment target is iOS 17.

For a physical device, choose your development team in Signing & Capabilities. The project contains the Prism Roll bundle and team identifiers; a separate publisher needs its own signing, App Store, and advertising configuration. Version **1.1.0 (15)** has a verified production archive built on GitHub with Xcode **26.6 (17F113)** and the **iOS 26.5 SDK**, followed by local distribution signing and export. Apple processed it as **VALID** and **APP_STORE_ELIGIBLE**. Upload, processing, and review status are tracked in [VALIDATION.md](VALIDATION.md#build-15-app-store-review).

Build 15's review submission contains **21 items**: the app version and all 20 Game Center components. The live version remains 1.0.0 (12) while review is pending. Production AdMob identifiers and published privacy messages are configured; sample-ad requests and US opt-out were exercised, while production serving and European consent delivery remain unverified. The production archive contains the app and bundled SDK privacy manifests. The public [support](https://thoughtahead.com/prism-roll/support.html) and [privacy policy](https://thoughtahead.com/prism-roll/privacy.html) URLs are verified. The hosted policy and App Store privacy disclosures were updated on September 16 for this release, including optional crash diagnostics. See [VALIDATION.md](VALIDATION.md#build-15-app-store-review) for release evidence and service limits.
...
[Build 15's successful run](https://github.com/jonluca/amaze/actions/runs/35163482422) archived source commit `b8ba1f8b499d1ab1f2bbcbfd1040718e210d7b18` with the same pinned production toolchain. The signed build and its review submission are recorded in [build 15 verification](VALIDATION.md#build-15-app-store-review).
...
Version 1.0.0 (12) is now live on the App Store. Full ad serving also requires AdMob store linking, app-ads.txt verification, and Google readiness approval; public availability alone does not verify those account settings or establish the cause of every no-fill response. [Google app readiness](https://support.google.com/admob/answer/10564477?hl=en), [app verification](https://support.google.com/admob/answer/14538460?hl=en).

The hosted [app-ads.txt](https://thoughtahead.com/app-ads.txt) returns HTTP 200 with the correct publisher entry. The live App Store marketing URL is already `https://thoughtahead.com/prism-roll/support.html`, so its developer-website domain matches; Google uses that marketing URL to discover app-ads.txt. The saved September 6 account record (`release/admob-configuration.json`) said verification was pending. Current account approval is unverified because browser security policy blocked the authenticated AdMob refresh. Verify store linking and app verification in AdMob, then confirm Google reports the app ready. [Google developer-website setup](https://support.google.com/admob/answer/9363762?hl=en).
...
| `PrismRoll/Services` | StoreKit entitlements, Game Center reporting and activities, and dormant Duel support |
| `PrismRoll/Diagnostics` | Optional crash reporting, consent, and sanitized error categories |

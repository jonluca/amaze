...
- Hide ad-dependent reward buttons while inventory is loading or unavailable; show “Watch ad” only when `canShowRewarded` is true. First-maze free hints remain available with a “First maze” caption. Existing lifecycle calls to `prepare()` retry inventory without a visible retry button. Keep `statusMessage` available in Settings for more detail.

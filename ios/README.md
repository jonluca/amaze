...
**1.0.0 (13)** is available in the private **Owner Testing** TestFlight group, verified **September 9, 2026 at 4:12 PM Pacific**. Continuous dragging now requires clearer, more deliberate turns and ignores release movement that could add an accidental second action. See [release verification](VALIDATION.md#build-13-testflight-release).

The App Store version remains on **build 12**, **Waiting for Review** with all four in-app purchases. Build 12 added 1,000 unique Classic boards with bundled exact Perfect counts, asynchronous optimal hints from any current paint state, and continuous drag turns and reversals. The unreleased catalog has no historical-board preservation layer. Release remains manual after approval. See [submission verification](VALIDATION.md#build-12-testflight-and-app-store-review).
...
For a physical device, choose your development team in Signing & Capabilities. The project contains the Prism Roll bundle and team identifiers; a separate publisher needs its own signing, App Store, and advertising configuration. Prism Roll's App Store record and No Ads product have been created. Version **1.0.0 (12)** has a verified production archive built on GitHub with Xcode **26.6 (17F113)** and the **iOS 26.5 SDK**, followed by local distribution signing and export. Apple processed it as **VALID** and **APP_STORE_ELIGIBLE**. Upload, processing, and review status are tracked in [VALIDATION.md](VALIDATION.md).

Build 12 is **In Beta Testing** in the private **Owner Testing** group. Its sole existing tester retains access, and automatic notifications are enabled. Build 12, No Ads, and all three coin packs are **Waiting for Review**. Existing localization and reviewer contacts were preserved, and review notes describe the current controls. Release remains manual after approval. Production AdMob identifiers and published privacy messages are configured; sample-ad requests and US opt-out were exercised, while production serving and European consent delivery remain unverified. The production archive contains the app and both Google SDK privacy manifests. The public [support](https://thoughtahead.com/prism-roll/support.html) and [privacy policy](https://thoughtahead.com/prism-roll/privacy.html) URLs are verified; see [VALIDATION.md](VALIDATION.md) for release evidence and service limits.

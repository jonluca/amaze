...
struct PrivacyPolicyView: View {
    private let attribution = AppsFlyerAttributionService.shared
    ...
    var body: some View {
        Form {
            ...
            policySection("Optional usage analytics", text: "If you choose to share usage data, Prism Roll uses Google Analytics for Firebase to understand sessions, returning players, screens, modes, level outcomes, hints, rewards, collection choices, purchase activity, and which campaigns bring people to the app. This can include Apple Ads attribution information and verified purchase information such as product, value, currency, and transaction-derived fields. Analytics also measures ad impressions and associated revenue when available. Analytics uses a random app-instance identifier and app/device information; Google may derive approximate location from network information. We do not send your name, email, Game Center identity, saved maze layout, or payment-card details. Analytics advertising-identifier and vendor-identifier collection are disabled, and analytics advertising consent remains denied.\n\nAnalytics starts only after you choose Share usage data. You can decline and keep playing, or change Share usage analytics in Settings. Turning it off stops future analytics collection and resets the Firebase Analytics identifier on this device; it does not delete reports already received by Google. New purchase reporting is limited to purchases made during your current participation period; restoring an older purchase does not report new revenue. A local record prevents the app from reporting a handled transaction again. This choice is separate from advertising privacy choices.")

            if attribution.isAvailable {
                policySection("Optional install attribution", text: "If you separately enable Share install attribution while Share usage analytics is on, Prism Roll uses AppsFlyer to measure installs, app sessions, and which campaigns bring people to the app. AppsFlyer receives installation and session information, attribution information when available, app and device information, IP address, and an AppsFlyer installation identifier. Approximate location may be derived from network information. We do not provide your name, email, Game Center identity, purchase or ad revenue, payment-card details, saved maze, or challenge links to AppsFlyer. AppsFlyer advertising-identifier and vendor-identifier collection, personalized advertising, and sharing with advertising partners are disabled.\n\nExisting usage-analytics choices do not enable AppsFlyer. The first-launch Share usage data choice can enable both providers when both are described on that screen. You can change Share install attribution in Settings. Turning off either install attribution or usage analytics stops new AppsFlyer measurement; it does not delete information already received by AppsFlyer or reset its installation identifier. This choice is separate from crash reporting and advertising privacy choices.")
            }
            ...
            policySection("Game Center", text: "If you sign in to Apple's Game Center, Prism Roll sends achievement progress and completed-maze scores to Apple. Your Game Center profile and scores may be visible to other players according to your Game Center settings. The app keeps a local report queue associated with your game-specific player identifier so saved progress can sync after a connection failure. Your first Game Center account on this installation can receive your existing local progress; switching accounts keeps their new earned progress separate.\n\nThe Apple Games app can open Classic, Time Rush, or the daily maze. Game Center does not back up or restore your maze saves, coins, or collection. You can continue playing solo while signed out and manage Game Center sign-in, your profile, and sharing through iOS Settings. Game Center identifiers and profile names are not sent to Google Analytics or Crashlytics.")
            ...
            Section("Provider privacy information") {
                ...
                if attribution.isAvailable {
                    policyLink("AppsFlyer services privacy policy", url: "https://www.appsflyer.com/legal/services-privacy-policy/")
                }
                policyLink("App Store & Privacy", url: "https://www.apple.com/legal/privacy/data/en/app-store/")
                Text("These providers describe retention, deletion, and account controls in these policies. Removing Prism Roll does not delete records those services maintain. Requests concerning their services can be directed through those resources. These links require an internet connection.")

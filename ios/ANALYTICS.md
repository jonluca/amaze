...
The existing **Prism Roll acquisition tracking** task automation runs daily at **09:00 America/Los_Angeles**, checking Apple report changes, initial website reporting receipt, BigQuery export readiness, and AdMob review. It stays quiet for unchanged or non-actionable state. It does not enable billing, launch campaigns, or submit app releases.

AppsFlyer is not installed: no account/dev key is available, and the decision to use multiple paid networks remains pending. Its welcome install-attribution allowance expires after one year or 12,000 conversions; the ongoing free Zero tier covers clicks and impressions, not install attribution. Avoid adding a second attribution SDK until there is a concrete paid-network need and an agreed plan. No advertising spend or billing was enabled in this setup.

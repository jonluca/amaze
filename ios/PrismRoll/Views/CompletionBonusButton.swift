...
struct CompletionBonusButton: View {
    ...
    var body: some View {
        Button {
            ...
            Label("+50 bonus coins · \(ads.rewardedAvailability.caption)", systemImage: "play.rectangle")
                .font(.subheadline)
                .frame(minHeight: 44)
        }
        .buttonStyle(.borderless)
        .disabled(ads.isPresenting || ads.isPrivacyFormPresenting || store.isRewardPending || ads.rewardedAvailability == .loading)
        .accessibilityLabel("\(mode == .timed ? "Round" : "Level") \(number) bonus, 50 coins, \(ads.rewardedAvailability.caption)")
        .accessibilityHint(ads.rewardedAvailability.accessibilityHint)
